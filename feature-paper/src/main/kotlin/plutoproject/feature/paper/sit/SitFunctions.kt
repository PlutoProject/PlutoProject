...
val armorStandDataKey = NamespacedKey(plugin, "sit_passenger")

private suspend fun updateSit() {
    SitManager.sitters.forEach {
        val player = it
        val armorStand = player.seat ?: return@forEach // 有时可能玩家已经站起来了，但是异步任务仍然尝试获取实体
        player.withSync {
...
private suspend fun updateChunk() {
    plugin.server.onlinePlayers.forEach {
        val chunk = it.chunk
        chunk.withSync {
            clearIllegalArmorStands(chunk)
        }
    }
}

fun runSitCheckTask() = runAsync {
    while (!disabled) {
        updateSit()
        updateChunk()
        delay(2.seconds)
...
fun tryToStand(event: EntityEvent, needCancel: Boolean = true) {
    ...
    if (event is Cancellable) event.isCancelled = needCancel
    entity.stand()
}
...
fun handleSitLocationBroke(event: Event) {
    ...
    when (event) {
        is BlockEvent -> {
            locations.add(event.block.location.viewAligned())
            when (event) {
                is BlockPistonExtendEvent -> event.blocks.map { it.location.viewAligned() }.toCollection(locations)
                is BlockPistonRetractEvent -> event.blocks.map { it.location.viewAligned() }.toCollection(locations)
                is BlockExplodeEvent -> event.blockList().map { it.location.viewAligned() }.toCollection(locations)
            }
        }

        is EntityExplodeEvent -> {
            event.blockList().map { it.location.viewAligned() }.toCollection(locations)
        }

        is EntitySpawnEvent -> {
            if (event.entity.type == EntityType.FALLING_BLOCK) {
                val location = event.location.viewAligned()
                if (location.viewAligned().isSitLocation) {
